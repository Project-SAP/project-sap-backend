...
import { ClientToServerEvents, ServerToClientEvents, QueueRequestData, SocketData, Message } from './models/events.model';
            ...
            socket.on("joinQueue", (data: QueueRequestData) => {
                ...
                if(data.profile === "venter"){
                    ...
                    if(availableListeners.length > 0){
                        ...
                        const matchedEntry: SocketData = availableListeners.shift();
                        ...
                        const idx = findIndexByName(newEntry.name, availableVenters);
                        if(idx === -1){
                    ...
                    if(availableVenters.length > 0){
                        ...
                        const matchedEntry: SocketData = availableVenters.shift();
                        ...
                        // Check if user is already in queue
                        const idx = findIndexByName(newEntry.name, availableListeners);
                        if(idx === -1){
